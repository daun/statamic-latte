...
- **Blink paginator hand-off**: Statamic flattens paginated results to a plain array and stashes the real paginator in `Blink` under `'tag-paginator'`. `fetchTag` calls `Blink::forget('tag-paginator')` BEFORE `$tag->fetch()` and forgets it again after recovery. If you invoke Statamic tags without going through `Tags::fetch`, pagination silently degrades to a keyed array — and a stale Blink slot from an earlier tag on the same request can masquerade as "this tag returned a paginator".
