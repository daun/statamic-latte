...
- **"my bard/markdown field prints literal `<p>` tags as text"** → auto-escaping applies to every printed value, but fields whose fieldtype renders HTML (bard, markdown, redactor) are marked safe by `Content::wrapValue()` and print as markup. Seeing tags means the marking was lost: a modifier ran on the value, or it never came from an augmented field. `|noescape` still works.
