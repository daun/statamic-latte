...
- Only `{block}` / `{import}` belong directly inside `{embed}` — except loose content, which becomes the embedded file's `{block default}` (combining loose content with an explicit `{block default}` is a compile error).
