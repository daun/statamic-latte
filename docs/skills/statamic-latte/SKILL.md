---
name: writing-statamic-latte-templates
description: Author and edit Latte templates (.latte files) in Statamic sites using the daun/statamic-latte addon. Covers calling Statamic tags from Latte ({s:...} tags, (s:...) expressions, s() function), Statamic modifiers as filters, entry data access (Content/Deferred wrappers), Blade-style dot view resolution, layout resolution from entry data, {section}/{yield}, {slot}, inline {antlers}, and {cache}/{nocache}. Use when writing, reviewing, or debugging .latte templates in a Statamic or Laravel project, or when Statamic tags, modifiers, or entry data are used from Latte.
---
...
**This skill covers only the Statamic/Laravel-specific deltas.** Standard Latte 3.x — tag syntax, n:attributes, filters, blocks and inheritance, the expression language, escaping — is assumed known (see the `writing-latte-templates` skill or latte.nette.org).

Details are one level deep:

- **[statamic-tags.md](statamic-tags.md)** — the `{s:...}` tag bridge: all three spellings, argument syntax, `$value` scope, `as:`/`content:` params, pagination, forms, blocked tags, per-tag notes
- **[data.md](data.md)** — what template variables hold: `Content`/`Deferred` wrappers, resolve helpers, and a symptom → cause table
- **[views-and-composition.md](views-and-composition.md)** — view name resolution, automatic layouts, `{section}`/`{yield}`, `{slot}`, `{antlers}` interop
- **[caching.md](caching.md)** — `{cache}` fragment caching and `{nocache}` static-cache hole punching
...
**Layouts are automatic**, like Antlers: a page template just defines `{block}`s — no `{layout}` tag — and the addon wraps it in `resources/views/layout.latte`, or whatever the entry/collection sets via its `layout:` key. See [views-and-composition.md](views-and-composition.md).
...
Values augment lazily and stringify on print — `{$title}`, `{$author->name}` just work. **No method calls**: `$entry->title()` fails (`Content` has no `__call`). Unknown field handles return `null` silently. Test relationships with `{if $related}`, never `{ifset}`. Details and pitfalls: [data.md](data.md).

## Statamic tags in one minute

Three spellings, full reference in [statamic-tags.md](statamic-tags.md):
...
4. `{if $related}` is the correct emptiness test for relationships; `{ifset}` gives shape-dependent wrong answers ([data.md](data.md)).
5. `$entry->title()` fails — no method dispatch on wrapped data. View data is read-only; never assign to it.
6. `form:create` returns **data, not `<form>` HTML**; `form:errors` is a **boolean gate**, not an error iterator ([statamic-tags.md](statamic-tags.md)).
...
10. Keep `{yield}` and `{nocache}` **outside** `{cache}` blocks, and note a `{cache}` block whose output is `''`/`'0'` never caches ([caching.md](caching.md)).
